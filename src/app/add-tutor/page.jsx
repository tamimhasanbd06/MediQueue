...
import { FaUser, FaImage, FaBook, FaClock, FaMoneyBill, FaCalendar, FaUniversity, FaMapMarkerAlt, FaChalkboardTeacher, FaLayerGroup, } from "react-icons/fa";
...
export default function AddTutorPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const creatorData = session?.user ? { id: session.user.id, name: session.user.name, email: session.user.email, image: session.user.image || "", key: "user", }
        : { key: "system", };
  ...
  return (
    <div className=" min-h-screen py-10 px-4 transition-all duration-500 bg-gray-50 dark:bg-linear-to-br dark:from-black dark:via-[#020817] dark:to-blue-950">
      ...
      <ToastContainer position="top-right" autoClose={2000} toastClassName={() =>
        ...
        >
          ...
          <div className="md:col-span-2">
            <button type="submit" disabled={loading}
              className=" w-full py-4 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg transition-all duration-300 hover:scale-[1.01] disabled:opacity-60">
